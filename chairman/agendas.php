...
$agendas = $conn->query(
    "SELECT a.AgendaId, a.Title, a.Description, a.createdAt, m.Title AS MeetingTitle
     FROM agendas a
...
<div class="wrap">
    ...
    <div class="card">
        <?php if ($agendas && $agendas->num_rows > 0): ?>
            <?php while ($agenda = $agendas->fetch_assoc()): ?>
                <div class="row">
                    <h3><?= htmlspecialchars($agenda['Title']) ?></h3>
                    <div class="meta"><?= htmlspecialchars($agenda['MeetingTitle']) ?> • <?= htmlspecialchars(date('d M Y', strtotime($agenda['createdAt']))) ?></div>
                    <p><?= htmlspecialchars($agenda['Description'] ?? 'No description provided.') ?></p>
                </div>
            <?php endwhile; ?>
        <?php else: ?>
